#[cfg(test)]
mod tests {
    use super::super::{make_ws_url, next_backoff, redact_api_key};
    use super::super::handlers::{init_cache_in_background, handle_sessions_event, handle_userdata_changed_event};
    use crate::client::{MediaClient, SessionInfo, NowPlayingItem, PlayState, UserDataChangedInfo, UserDataEntry};
    ...
    fn test_redact_api_key() {
        ...
        assert_eq!(redacted, "ws://localhost/socket?api_key=[REDACTED]&deviceId=test");
    ...
    async fn test_handle_sessions_event() {
        ...
        let client = Arc::new(MediaClient::new("http://localhost".to_string(), "key".to_string(), false));
        let config = Config {
        ...
        let st = state.lock().await;
        assert!(st.active_sessions.contains_key(&("test_server".to_string(), "session_1".to_string())));
    }
